...
使用Github OAuth可以进行push/pull 等等操作，而不需要输入用户名密码。但这需要事先在 Github 上创建一个 token：
...
切记，这个 token 的权限很大，不要把原文提交到 Github 上。

## 在github中生成Access Token

要想让Travis-CI可以去操作我们的repo，就需要给它权限。

办法是生成  `Access Token`。

首先进入github你的个人设置页面,：

![githubsetting](http://7xiym9.com1.z0.glb.clouddn.com/githubsetting.png)

如图，选择 `Settings` 。

在左侧面板中，可以看到 `Developer settings`，选择其中的 `Personal access tokens`. 按下 `Generate new token`，就可以生成我们要的token了。

如图：
![](http://7xiym9.com1.z0.glb.clouddn.com/githubaccesstoken.png)

## Travis-CI命令行工具,加密token

我们在写入 travis 配置时要先对这个 token 进行加密。
...
而细心的同学会发现，我们themes文件夹下的主题其实是一个`submodule` ，`themes/yourtheme/` 就是一个包含.git/的子项目仓库。可以及时去同步更新。传到github上的话，会看到它的文件夹是       `灰色` 的,代表一个git链接。点击后，会调转到对应git仓库。

如下图：
![themes](http://7xiym9.com1.z0.glb.clouddn.com/hexothemes.png)
