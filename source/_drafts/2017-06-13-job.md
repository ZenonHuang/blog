...
runtime 注册和修改类 ，交换方法和添加方法ORIGforwardInvocation指向原始的实现IMP。替代方法走 objc_msgForward 转发，类的forwardInvocation:方法的实现被替换为JPForwardInvocation的实现，然后在转发方法里进行操作。
...
hook 分类，在类似willDisappear 中，调用一个 willDealloc 方法，执行大概 3 秒延迟的block, weakself 调用 方法，进行断言，已经释放，则 nil 无法调用到断言。
...
作者：AmumuHandsome
链接：https://www.jianshu.com/p/419e3b081e5a
來源：简书
